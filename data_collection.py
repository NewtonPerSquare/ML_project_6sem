...
def load_config():                                          #Загружает конфигурацию из YAML
    with open('config.yaml', 'r') as f:
        return yaml.safe_load(f)


def prepare_directories(config):                            #Создает директории для данных
...
def process_data(input_path, raw_data_dir, metadata_dir):   #Минимальная обработка и формирование батчей
    ...
    #Разбивка на батчи по месяцам
    df['OrderMonth'] = df['Order Date'].dt.to_period('M')
    batches = df.groupby('OrderMonth')

    for month, batch in batches:
        batch_file = os.path.join(raw_data_dir, f'batch_{month}.csv')
        batch.drop('OrderMonth', axis=1).to_csv(batch_file, index=False)

        #Метаданные (ого)
        metadata = {
            'batch_id': str(month),
            'num_records': int(len(batch)),
            'delivery_time_stats': {
        ...
        #Запись в JSON
        metadata_file = os.path.join(metadata_dir, f'metadata_{month}.json')
        with open(metadata_file, 'w') as f:
            json.dump(metadata, f)

        logging.info(f"Сохранен батч {month} с {len(batch)} записями.")
